...
本 harness **不做训练**，但为 Agentic RL 提供了完整的数据工程基础——从事件收集到轨迹导出：

**Trace（细粒度，逐事件）→ Trajectory（聚合，可训练）两层记录**：

| | Trace (jsonl) | Trajectory (json) |
|---|---|---|
| 文件 | `data/traces/<runId>.jsonl` | `data/trajectories/<runId>.json` |
| 粒度 | 逐 `AgentEvent` | 聚合摘要 |
| 用途 | Debug、回放、审计 | RL 训练、评估分析 |
| 格式 | 一行一个 JSON 事件 | 单个 JSON 对象 |

核心文件：
- `src/runtime/events.ts` — `AgentEvent` 联合类型（11 种事件），所有轨迹数据的源头
- `src/runtime/trace.ts` — `TraceWriter`：在 `runAgentLoop` 第一步创建，对每个 `AgentEvent` 写入一行 jsonl
- `src/trajectories/trajectory.ts` — `AgentTrajectory` 类型：`runId` / `userInput` / `toolCalls[]`（每次工具调用的名称/参数/成功/摘要）/ `modelMessagesSummary[]` / `finalAnswer` / `success`（`stoppedReason === "final"`）/ `humanFeedback`（预留）/ `usage`（token 消耗）/ `errors[]`
- `src/trajectories/exporter.ts` — `TrajectoryRecorder`：在 loop 创建时启动，对每个 `AgentEvent` 调用 `observe(event)` 自动累积，`done` 事件时写入 JSON 文件
- `src/security/secrets.ts` — `redactValue()`：轨迹写入前自动脱敏 API key、`sk-*` token、Authorization Bearer、PEM 私钥

数据可用于 RL pipeline：从 `data/trajectories/*.json` 提取 `success === true` 的轨迹 → 清洗 → 导出为 RLHF/DPO 训练格式（JSONL）。`humanFeedback` 预留字段可直接对接标注流程。
