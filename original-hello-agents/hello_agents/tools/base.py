...
def tool_action(name: Optional[str] = None, description: Optional[str] = None):
    ...
    def decorator(func: Callable):
        setattr(func, '_is_tool_action', True)
        ...
        for name, method in inspect.getmembers(self, predicate=inspect.ismethod):
            if hasattr(method, '_is_tool_action'):
                tool = AutoGeneratedTool(
                    parent=self,
                    method=method,
                    name=getattr(method, '_tool_name', ''),
...
class LightweightTool(Tool):
    """兼容教程的轻量级工具基类 — 不强制 ToolResponse 返回类型。

    教程演示工具（MemoryTool, RAGTool, NoteTool, TerminalTool 等）
    直接返回 str/dict，而非 ToolResponse。此基类重写 run_with_timing
    以兼容该简化协议，同时保持 Tool 的类型身份。
    """

    def get_parameters(self) -> List[ToolParameter]:
        return []

    def run_with_timing(self, parameters: Dict[str, Any]) -> Any:  # type: ignore[override]  # LightweightTool 简化协议：返回 Any 而非 ToolResponse
        try:
            return self.run(parameters)
        except Exception:
            import json as _json
            return _json.dumps({"error": "tool execution failed"})


class AutoGeneratedTool(Tool):
    """自动生成的工具 - 从方法签名和 docstring 自动提取参数"""

    def __init__(self, parent: Tool, method: Callable, name: Optional[str] = None, description: Optional[str] = None):
        """初始化自动生成的工具
